...
- 本地网页控制面板入口为 `scripts/run_control_panel.py`，默认访问 `http://127.0.0.1:8765`。
- 如果从隔离 worktree 启动面板代码，应使用 `py -3.11 scripts\run_control_panel.py --project-root E:\自动化\财务`；这样面板读取真实项目配置、账号池和输出目录，并调用真实项目的 `main.py`。
- 控制面板不替代现有总调，只生成并运行现有 `main.py` 参数；业务人员可按环境、平台、模块、账号、店铺筛选、周期和诊断模式发起任务。
- 页面布局原则：左侧只放“运行范围”筛选和启动按钮；右侧主区域用 `运行日志` / `定时计划` Tab 切换，不再把定时表单塞进侧栏。
- 账号列表展示按账号名称去重；同一账号出现在 `shein_main_12`、`shein` 等多个账号源时只显示一次。
- 面板日志只展示业务可读文本，隐藏 `[auth]`、webdriver、DrissionPage 等技术噪音；完整终端日志仍保存在 `output/panel/runs`。
...
- 定时计划触发的运行记录展示名使用“计划名称 - 触发时间”，例如 `temu月度财务 - 2026-06-02 23:00`，便于业务人员从运行记录直接对应计划。
- 左侧提供“中止目前任务”按钮，只终止当前由面板启动并仍在运行的子进程；终止后运行记录状态为 `stopped`，业务日志补写“用户已中止当前任务”。
- 面板业务日志、失败明细和无数据明细使用任务配置的 `task_name`，原始磁盘日志仍保留任务 ID，兼顾业务可读性和技术排查。
- 左侧账号列表按平台全部账号源展示；选定账号后，模块按 `account_source` 禁选不适用项，并按 `frequency` 禁选不支持当前周期的项。“全选模块”只选择可用模块。
- 手动运行必须至少选择一个账号和一个可用模块，后端会再次校验账号、模块和周期组合，避免空账号筛选被底层解释为运行全部账号。
