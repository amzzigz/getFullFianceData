...
- 控制面板最终失败/无数据明细将任务 ID 映射为 `task_name` 中文名；账号列表与模块勾选解耦，模块按账号源和周期即时禁选，后端同步拒绝非法或空选择。
- 控制面板运行记录新增 `display_name`：手动运行默认显示运行 ID，定时计划触发显示“计划名称 - 触发时间”，例如 `temu月度财务 - 2026-06-02 23:00`。
- 左侧栏新增“中止目前任务”按钮和 `/api/runs/active/stop` 接口；面板会终止当前子进程树，运行记录标记为 `stopped`，并在日志中写入“用户已中止当前任务”。
- 新增控制面板回归测试，覆盖计划运行记录命名和中止当前任务。
